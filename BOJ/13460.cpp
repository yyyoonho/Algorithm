...
void Move(int &x, int &y, int dx, int dy, int &mvCnt){
...
void BFS(){
	while(!Q.empty()){
		...
		if(d>=10) break;
		for(int i=0; i<4; i++){
			int nextRx=rx,nextRy=ry,nextBx=bx,nextBy=by,rc=0,bc=0;
			Move(nextRx, nextRy, dx[i], dy[i], rc);
			Move(nextBx, nextBy, dx[i], dy[i], bc);
